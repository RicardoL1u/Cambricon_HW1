...
class SoftmaxLossLayer(object):
    ...
    def forward(self, input):  # 前向传播的计算
        ...
        input_exp = np.exp(input - input_max) # TODO: WHY? delete input_max
        # 其中 X(i,j)代表 X 中对应第i样本j位置的值。当 X(i,j)数值较大时，求e指数可能会出现数值上溢的问题。
        # 因此在实际工程实现时，为确保数值稳定性，会在求 e 指数前先进行减最大值处理，
